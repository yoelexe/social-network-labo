...
!function(t2, e = true) {
  !function(t3) {
    P = t3;
  }(SDK_VERSION$1), _registerComponent(new Component("firestore", (t3, { instanceIdentifier: n, options: s }) => {
...
class PersistentConnection extends ServerActions {
  ...
  onDataPush_(action, body) {
    ...
    if (action === "d") {
      ...
      error$1("Unrecognized action received from server: " + stringify(action) + "\nAre you using the latest client?");
    }
  }
...
const addLike = (id2) => {
  ...
  rf(docRef, {
    likes: Sf(currentUser.uid)
  });
};
const removeLike = (id2) => {
  const currentUser = auth.currentUser;
  const docRef = rh(db, "post", id2);
  rf(docRef, {
    likes: Df(currentUser.uid)
  });
};
const loginConfig = (email, password) => new Promise((resolve, reject) => {
...
const loginWithTwitter = () => new Promise((resolve, reject) => {
  ...
  signInWithPopup(auth, provider).then((result) => {
    ...
    console.log("emailError : ", error2.email);
    console.log("errorMessage : ", error2.message);
    const codeError = error2.code;
...
const error = (navigateTo2) => {
  ...
  othermessage.textContent = "P\xE1gina no encontrada";
...
const muro = (navigateTo2) => {
  ...
  <button class='open-popup'>\xBFQu\xE9 receta quieres compartir hoy?</button>
  </div>
  ...
  onGetTasks((querySnapshot) => {
    ...
    btnEdit.forEach((btn) => {
      btn.addEventListener("click", async (event) => {
        ...
        const formPost2 = muroDiv.querySelector(".form-post");
        formPost2["textarea-post"].value = taskEdit.description;
        editStatus = true;
